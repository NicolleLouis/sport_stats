...
class TagSuccessRepartitionStatsView:
    @classmethod
    def get(cls, request, climber_id):
        climber = UserRepository.get_by_id(climber_id).climb_profile
        data = TagRadarService.aggregate_tag_data(climber).export_success_data()
        chart = PieChart(
            raw_data=data
        )
        return render(request, 'single-chart.html', {
            'title': f'Piste réussies par tag pour: {climber.user.name}',
